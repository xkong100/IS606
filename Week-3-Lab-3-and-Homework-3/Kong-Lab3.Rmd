...
hist(mdims$hgt, xlab = "Male Height")

hist(fdims$hgt, xlab = "Female Height")
summary(mdims$wgt)
summary(fdims$wgt)
```
By the histograms, male and female heights are approximately symmetric.
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
Yes. It appears nearlt normal distribution.
...
The points lie on the line or very close to it.  
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    Yes, the normal probability plot looks similar to the data plots.
...
```{r}
    fhgtmean <- mean(fdims$hgt)
    fhgtsd   <- sd(fdims$hgt)
    taller <- 1 - pnorm(q=165, mean = fhgtmean, sd = fhgtsd)
    taller
    
    tallerTheory <- sum(fdims$hgt > 165) / length(fdims$hgt)
    tallerTheory
```

** What is the probability that a randomly chosen female is less than 60kg?
 ```{r}
    fwhgtmean <- mean(fdims$wgt)
    fwgtsd   <- sd(fdims$wgt)
    lighter <- pnorm(q=60, mean = fwhgtmean, sd = fwgtsd)
    lighter
    
    lighterTheory <- sum(fdims$wgt < 60) / length(fdims$wgt)
    lighterTheory
```
...
-   Now let's consider some of the other variables in the body dimensions data 
    ...
    **b.** The histogram for female elbow diameter (`elb.di`) belongs to normal 
    probability plot letter _***C***___.
    ...
    **d.** The histogram for female chest depth (`che.de`) belongs to normal 
    probability plot letter _***A***___.

-   Note that normal probability plots C and D have a slight stepwise pattern.  
    Why do you think this is the case?
It maps to the transition from a percentile to the next one.
