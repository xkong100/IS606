...
The cases are the information about the birth. There are `r nrow(nc)` cases in the data set.
...
highlight about the relationship between these two variable?
...
** H~1: There is a difference between mean weights of smokers and non-smokers.**
...
```
**The p-value is 0.826 that is greater than alpha, we fail to reject null hypothesis.

-   Now, a non-inference task: Determine the age cutoff for younger and mature 
mothers. Use a method of your choice, and explain how your method works.
```{r}
require(knitr)
young_mom <- subset(nc, nc$mature == "younger mom")
mature_mom <- subset(nc, nc$mature=="mature mom")
kable(head(young_mom))
kable(head(mature_mom))
summary(young_mom)
summary(mature_mom)
```
...
plain language.
```{r}
inference(y=nc$weight, x=nc$gender, est="mean", conflevel = 0.90, type = "ci", null= 0, alternative ="twosided", method="theoretical", order= c("female", "male"))
```
