...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    They are sample statistics. These values and proportions are based on sample data that are estimates of population parameters.
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    assumption?
    The smapling method has to be random. In a global survey, it might be difficult to become random because the different countries have different diversities.
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    Per country, % of religious, not religious, atheist and did not respond. Individual response of a person in a given country and the year the poll was taken.
...
7.  Using the `inference` function, calculate confidence intervals for the 
    ...
    inference(chn12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
    ```
    For Japan
    ```{r}
    jn12 <- subset(atheism, nationality == "Japan" & year == "2012")
    inference(jn12$response, est = "proportion", type = "ci", method="theoretical", success = "atheist")
    ```
...
It is parabolic. ME increases until P reaches its maximum at 0.5, then ME decreases.
...
-   Answer the following two questions using the `inference` function. As 
    ...
    HA: P12 - p05 != 0, there was change in the atheism index in Spain from 2005 to 2012.
...
inference(bul05$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
inference(bul12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```
```{r}
p_bul05 = 0.0505
...
PE_bul = p_bul12 - p_bul05

SE_bul = sqrt(((p_bul05*(1-p_bul05))/n_bul05)+((p_bul12*(1-p_bul12))/n_bul12))
SE_bul
```
```{r}
PE_bul + (1.96*SE_bul)
