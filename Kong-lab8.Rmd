...
1.  Is this an observational study or an experiment? The original research
    ...
    This is an observational study. We can not answer this question as it is            phrased. The instructor's beauty has a positive or negative correlation to the      student course evaluation.
...
2.  Describe the distribution of `score`. Is the distribution skewed? What does 
    ...
    hist(evals$score)
    ```
    The evaluation scores are skewed to the left. Students have more positive evaluations than negative evaluations. 
...
4.  Replot the scatterplot, but this time use the function `jitter()` on the
    ...
    mbty <- lm(score ~ bty_avg, data=evals)
...
5.  Let's see if the apparent trend in the plot is something more than
    ...
    ```{r}
    plot(x=mbty$residuals, y=evals$bty_avg)
    abline(h = 0, lty = 3)
    hist(mbty$residuals)
    ```
...
8.  Is `bty_avg` still a significant predictor of `score`? Has the addition
    ...
    ```{r}
    m_bty_gen <- lm(score ~ bty_avg + gender, data = evals)
    m_bty_gen2 <- lm(score ~ bty_avg, data= evals)
    summary(m_bty_gen)
    summary(m_bty_gen2)
...
11. Which variable would you expect to have the highest p-value in this model? 
    ...
    number of professors as the variable to have the least association with the professor's evaluation score.
...
The p-value is 0.11 means thant it has a weak positive relationship.
...
14. Drop the variable with the highest p-value and re-fit the model. Did the
    ...
    ```{r}
    m_back <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval + cls_students + cls_level + cls_credits + bty_avg + pic_outfit + pic_color, 
    data = evals)
summary(m_back)
```
...
No. Classes should be independent.
18. Based on your final model, describe the characteristics of a professor and 
    course at University of Texas at Austin that would be associated with a high
    evaluation score.
    The professor should graduate from an Aerican school and teaches at least 1 credit course. He is male and not a minority. He is relatively young, and he has a high beauty average score 

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?
No. Since the sample size of 6 is too small. The beauty is culturally biased.
<div id="license">
