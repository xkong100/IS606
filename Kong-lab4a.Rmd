...
The sample distribution is skewed to the right. The sample can represent the population distribution.
...
When we take the sample of 50, our mean is very close to mean of sample 1. 
If we take two more samples, the size 1000 will be more accurate than the size of 100.
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    There are 5000 elements in the sample. It becomes approximately normal distribution, the highest frequency is approaching to the middle point. If we collected 50,000 sample means, it will get very close to the population mean.
...
There are 100 elements. The 100 elements represent the sample mean.
...
The data values cluster to the center, and the spread become smaller.
* * *
## On your own
...
-   Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    Distribution 3 has a smaller spread. We would prefer a distribution with a small spread.
